...
We evaluate this interface in motionless and motion mode. For motionless mode, it can achieve 82.4\% accuracy for classifying different kinds of finger postures. The system can distinguish swipe gestures with high accuracy (93.2\%).
...
% TODO delete this sentence. but need for another? -- NAILDISPLAY HAS THIS SENTENSE
...
\subsection{Hardware}
Based on pilot study, we developed \projectName{} using a 3$\times$3 array of 120-ohm 0.2-mm strain gauges for sensing part (Shown in \autoref{fig:main}). Below the strain gauges, we used a stretchable and flexible artificial-skin to adhere sensors onto user's fingernail. The size of it only takes 1.0cm$\times$1.1cm which is smaller than 1 US cent, and the thickness of it is 1mm. Each of the strain gauge is directly wired to the computing part.
The computing hardware is consisted of an Arduino Nano board, two 8-to-1 analog switches(MAX4617, Maxim Integrated), a dual digital potentiometer (AD5231, Analog Devices), and two instrumentation amplifiers (AD623, Analog Devices and INA122U, Texas Instruments).
% hardware confirmed!!
...
% TODO 跑10fold cross user的results 
