...
uTrack\cite{uTrack} and FingerPad\cite{FingerPad} add on the fingertip a small magnet, and uses a magnetic field to track finger movements. TouchSense\cite{TouchSense} uses 3-axis accelerometer to detect finger postures for switching different modes of input.
...
The diagram of the computing hardware is shown in \autoref{fig:completeCircuitDiagram}. First, the multiplexers are sequentially selected to connect one of the strain gauges. Once one of the strain sensors is selected, it becomes one of the four resistors on the Wheatstone bridge. When the forces apply, the strain of fingernail let the sensor slight bend or contour which caused the ohm value of strain gauges lower or higher. Due to the changes of ohm value, one side of the voltage in Wheatstone bridge also changed.
% TODO 怪怪的
...
Since the difference of the input data sets, we implemented two algorithm specifically for swipe gestures (Motion mode) and finger posture (Motionless mode).
...
For each mode, we scale each modes' feature to the range of -1 to 1 and then train a multi-class SVM classifier with a Radial Basis Function (RBF) kernel.
%\begin{figure*}[bp]
% \begin{center}
%  \setcounter{figure}{5}
