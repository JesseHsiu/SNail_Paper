...
The apparatus is shown in \autoref{fig:apparatus}. We used the same load-cell sensor (Error: $\pm 0.5$ grams) as our pilot study. In this experiment, we instrumented a 9 degree of freedom (9DOF) sensor on user's index finger. Since strain gauge is very sensitive to the strain, we avoid 9DOF sensor to contact fingernail. The 9DOF sensor is only used for checking whether user is performing the applicable angle for collecting data. The surface we used consists of medium density fibreboard.
% TODO 要把那個板子說的更明確一些
...
% TODO 有一點重複？ => Once posture or pressure is inapplicable during the study, recorder will automatically skip the data.
...
The accuracy for each force level is shown in \autoref{fig:postureResult}, which shows that the higher pressure level the better accuracy gets. If we drop 0.4N and 0.6N data, accuracy can reach 82.44\% (SD: $\pm$12.74\%). Cross-user evaluation using 10-fold cross-validation was computed.
...
\textit{Sensing from all fingers:} Mounting the device on all our fingernails is also our future work. Since more fingers are evolved, we can enable more functions such as multi-touch input like pinch and zoom. 
% TODO 可以討論多一點，然後是否要回報cross user的數據
%黏不緊的問題
