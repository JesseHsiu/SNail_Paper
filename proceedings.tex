...
\maketitle
\begin{abstract}
We present \projectName{}, a nail augmented device that senses user's fingernail contour and bend when force is applied on a surface. Using an array of 0.2-mm strain gauges, \projectName{} is small enough to fit on fingernails, and at the same time, flexible and stretchable.
We evaluate this interface in motionless and motion mode. For motionless mode, it can achieve 82.4\% accuracy for classifying different kinds of finger postures. Switching to motion mode, swipe gestures are recognized with high accuracy (93.2\%).
% Since the device is always available, 
The continuous availability of \projectName{} allows users to perform swipe gestures or touch postures on nearby surfaces to enable a variation of application usage.
We also show some example scenarios of using this technique such as smart TV and smart watches for extending input area and dimension.
